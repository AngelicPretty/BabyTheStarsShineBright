...
if ( ! function_exists( 'twentythirteen_post_nav' ) ) :
	...
	function twentythirteen_post_nav() {
		...
		<div class="post_navigation-center">		<ul class="post_fusion-shere list-inline">
			...
			<li class="list-inline-item post_fusion-shere-tw"><a href="https://twitter.com/intent/tweet?url= <?php echo $permalink ; ?>&amp;title= <?php echo $title ; ?>&amp&text=通信販売よりシステムメンテナンスのお知らせ/Webstore System Maintenance announcement.&tw_p=tweetbutton" target="_blank"><i class="fab fa-twitter"></i></a></li>
...
function content_header() {
		...
		$categories_list = get_the_category_list( __( ', ', 'twentythirteen' ) );
		if ( $categories_list ) {
			echo '&nbsp<span class="badge badge-pink" >' . $categories_list . '</span>&nbsp;';
		}
...
//目录分页
function babyssb_paging_nav( $range = 4 ) {
			...
			if( $paged != 1 ) {
			echo "<a href='".get_pagenum_link(1) ."' class='fal fa-angle-double-left' title=''></a>";
			}
			previous_posts_link('<i class="fal fa-angle-left"></i>');
			if ( $max_page >$range ) {
			if( $paged <$range ) {
			for( $i = 1; $i <= ($range +1); $i++ ) {
			...
			next_posts_link('<i class="fal fa-angle-right"></i>');
			if($paged != $max_page){
			echo "<a href='".get_pagenum_link($max_page) ."' class='fal fa-angle-double-right' title=''></a>";
			}
			echo '<span class="total">'.$max_page.'</span>';
			}
}
...
function baby_entry_meta() {
		...
		if ( $categories_list ) {
            /* <p class="post-category"><span class="badge badge-default">Shop Information</span></p> */
			$categories_title = '<span class="badge badge-pink" >' . $categories_list . '</span>&nbsp;';
			echo strip_tags("$categories_title","<span>");
		}
...
function replace_gravatar($avatar) {
	...
	"//1.gravatar.com/avatar/", "//2.gravatar.com/avatar/", "//cn.gravatar.com/avatar/"), "//0t.net.cn/avatar/", $avatar);
	return $avatar;
}
